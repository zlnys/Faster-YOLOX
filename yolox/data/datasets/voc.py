...
class VOCDetection(Dataset):
    ...
    def __init__(
        ...
        for (year, name) in image_sets:
            self._year = year
            rootpath = 'E:/Desktop/YOLOX-main/datasets/VOCdevkit/VOC2007'
            for line in open(
    ...
    def _do_python_eval(self, output_dir="output", iou=0.5):
        ...
        if iou == 0.5:
            ...
            plt.ylim([0.0, 1.05])
            plt.xlim([0.0, 1.05])
            color = ['r', 'g', 'b', 'c', 'm', 'y', 'k', '#A52A2A', '#8B008B', '#7CFC00']
            for i in range(len(classes_label)):
                plt.plot(classes_rec[i], classes_prec[i], label=classes_label[i], color=color[i], alpha=0.7)
            plt.legend(loc="lower left")
            plt.savefig("PR.jpg", dpi=300)
            plt.clf()
