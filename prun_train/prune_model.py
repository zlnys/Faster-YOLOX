...
class CSPDarknet_prune(nn.Module):
    def __init__(self,
        ...
        for p in range(base_depth):
            named_m_bottle_cv1_bn = named_m_base + ".m.0.conv1.bn".format(p)
            named_m_bottle_cv2_bn = named_m_base + ".m.0.conv2.bn".format(p)
            bottle_cv1in = chin[-1]
...
class yolox_prun(nn.Module):
    def __init__(self, mask_bn_channel_dict):
        ...
        head = YOLOXHead_prune(10, mask_bn_channel_dict)
