...
def save_whole_model(weights_path, num_classes):
    ...
    model.eval()
    torch.save(model, 'D:/deeplearning/YOLOX-main/YOLOX_outputs/1/whole_model.pth')
    print("保存完成\n")
...
def Conv_pruning(whole_model_weights):
    ...
    for k,v in model_dict.items():
        if k == 'backbone.backbone.model.features.0.1.weight':
            ...
            pruning_plan = DG.get_pruning_plan((model.backbone.backbone.model.features)[0].conv, tp.prune_conv, idxs=pruning_idx)
    ...
    torch.save(model, 'D:/deeplearning/YOLOX-main/YOLOX_outputs/1/Conv_pruning.pth')
    logger.info("剪枝完成\n")
@logger.catch
def layer_pruning(whole_model_weights):
    ...
    num_params_before_pruning = tp.utils.count_params(model)
    #可以对照yolox结构进行剪枝
    included_layers = list((model.backbone.backbone.model.features.modules()))#对主干进行剪枝
    ...
    #输出一下剪枝前后的参数量
    logger.info("Params:%s=>%s\n"%(num_params_before_pruning,num_params_after_pruning))
    torch.save(model, 'D:/deeplearning/YOLOX-main/YOLOX_outputs/1/layer_pruning.pth')
    logger.info("剪枝完成\n")


# model=torch.load('D:/deeplearning/YOLOX-main/YOLOX_outputs/1/yolox_s.pth')
# print('model',model)
...
weights_path = 'D:/deeplearning/YOLOX-main/YOLOX_outputs/1/yolox_s.pth'
num_classes = 10
save_whole_model(weights_path, num_classes)
Conv_pruning('D:/deeplearning/YOLOX-main/YOLOX_outputs/1/whole_model.pth')
